from function import find_recipes, suggest_alternatives, recipes_db, create_shopping_list
...
if selected_recipe:
    print(f"{selected_recipe_name} 레시피에 필요한 재료:")
    for ingredient in selected_recipe['ingredients']:
        # 재료가 사용자가 가지고 있는 재료인지 확인
        if ingredient in user_ingredients:
            print(f" - {ingredient} (가지고 있음)")
        else:
            # 가지고 있지 않은 재료에 대해 대체재를 제안
            alternatives = suggest_alternatives(ingredient)
            alternative_text = f"-- 대체재료: {', '.join(alternatives)}" if alternatives else "대체재료 없음"
